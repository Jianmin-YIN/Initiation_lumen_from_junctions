...
for T1=36
T2=39
Img1=imresize(uint8(imread(fileName,T1*2-1)),3);
Img2=imresize(uint8(imread(fileName,T2*2-1)),3);
...
probeX          =   linspace(30,300,ProbeNum);
probeY          =   linspace(30,270,ProbeNum);
...
[PredictPos,PredictVel,Credibility]=pivTrack(Img1,Img2,ProbePos,33); % PredictPos is in the form of [x1,y1;x2,y2];PredictVel is in the form of [vx1,vy1;vx2,vy2];In Credibility1 means reliable, 0 is not
Xp   =  ProbePos(:,1);  
Yp   =  ProbePos(:,2);
VX  =   PredictVel(:,1);
VY  =   PredictVel(:,2);
